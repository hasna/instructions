...
const MIGRATIONS = [
  ...
  `,
  `
  -- A target file has exactly one owning config. Older stores could contain
  -- several records for the same path; retain the record with the richest
  -- snapshot history and repoint profile membership before removing the rest.
  WITH ranked AS (
    SELECT
      c.id,
  ...
  ON CONFLICT (profile_id, config_id) DO UPDATE SET
    sort_order = MIN(profile_configs.sort_order, excluded.sort_order);
