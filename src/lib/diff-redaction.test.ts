...
describe("diffConfig — credential redaction", () => {
  ...
  test("never prints a pattern-less literal when the stored side is a placeholder", async () => {
    // Structural case: no value pattern matches and the key name is innocuous,
    // so ONLY the placeholder-vs-literal rule can catch this. If diff relies on
    // the pattern set alone, this test fails.
